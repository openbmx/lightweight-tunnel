...
func TestRoutingTable_FallbackFromDirectToServer(t *testing.T) {
	rt := NewRoutingTable(3)
	peer := p2p.NewPeerInfo(net.ParseIP("10.0.0.5"))

	peer.SetConnected(true)
	rt.AddPeer(peer)

	route := rt.GetRoute(peer.TunnelIP)
	if route == nil || route.Type != RouteDirect {
		t.Fatalf("expected direct route initially, got %#v", route)
	}
	...
	route = rt.GetRoute(peer.TunnelIP)
	if route == nil || route.Type != RouteServer {
		t.Fatalf("expected server route after fallback, got %#v", route)
	}
}

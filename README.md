...
**P2P技术特点**（使用UDP打洞，不是TCP打洞）：
- 🔧 **UDP打洞原理**: P2P连接使用UDP协议进行NAT穿透，而非TCP打洞
- ✅ **为什么用UDP**: TCP打洞成功率极低(<5%)，UDP打洞成功率高(60-95%)
- ✨ 更快握手速度（50ms 间隔，30 次尝试）
- ✨ 自适应保活（建立时 3 秒，稳定后 10 秒）
- ✨ 智能端口预测（覆盖 ±50 端口范围，优先顺序分配）
...
步骤1：客户端注册    A → [Server] ← B
步骤2：交换信息      A ← [Server] → B
步骤3：同时打洞      A ──UDP打洞──→ B  (使用UDP协议)
步骤4：P2P 建立      A ←──直连──→ B

技术说明：
- 主隧道(Server↔Client): 使用Raw Socket TCP伪装技术
- P2P连接(Client↔Client): 使用UDP打洞技术
- 为什么P2P用UDP不用TCP：TCP打洞成功率<5%，UDP打洞成功率60-95%
```
...
**关键技术特性**：
- ✅ STUN协议自动检测NAT类型
- ✅ 智能端口预测算法（优先顺序分配±10，扩展±50范围）
- ✅ 自适应握手策略（50ms快速尝试30次）
- ✅ 连续保活机制（建立期3秒，稳定后10秒可配置）
- ✅ 失败自动回退到服务器中转（无缝切换）
